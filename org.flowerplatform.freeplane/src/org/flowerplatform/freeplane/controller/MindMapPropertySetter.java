		...
		switch (property) {
			...
			case MIN_WIDTH:
				...
				if (newMinValue < 1) {
					...
					context.getService().setProperty(node, property, new StylePropertyWrapper()
					.setIsDefaultAs(true).setValueAs(MindMapConstants.DEFAULT_MIN_WIDTH), new ServiceContext<NodeService>(context.getService()));
					return;				
				} else {
		...
		if (addAdditionalSetPropertyUpdatesFor != null) {
			if (addAdditionalSetPropertyUpdatesFor.isEmpty()) {				
				for (Map.Entry<String, Object> entry : node.getOrPopulateProperties(new ServiceContext<NodeService>(context.getService())).entrySet()) {
					context.getService().setProperty(node, entry.getKey(), entry.getValue(),
				...
				for (String entry : addAdditionalSetPropertyUpdatesFor) {
					context.getService().setProperty(node, entry, node.getPropertyValue(entry),
					new ServiceContext<NodeService>(context.getService()).add(EXECUTE_ONLY_FOR_UPDATER, true));
				}
			}
		}
		...
		if (addAdditionalUnsetPropertyUpdatesFor != null) {
			if (addAdditionalUnsetPropertyUpdatesFor.isEmpty()) {
				for (Map.Entry<String, Object> entry : node.getOrPopulateProperties(new ServiceContext<NodeService>(serviceContext.getService())).entrySet()) {
					serviceContext.getService().unsetProperty(node, entry.getKey(),
					new ServiceContext<NodeService>(serviceContext.getService()).add(EXECUTE_ONLY_FOR_UPDATER, true));
				}	 
			} else {
