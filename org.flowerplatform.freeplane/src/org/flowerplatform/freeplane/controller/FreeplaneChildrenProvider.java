...
public class FreeplaneChildrenProvider extends ChildrenProvider {
	...
	public List<Pair<Node, Object>> getChildren(Node node) {
		...
		if (node.getId() == null) {
			return Collections.singletonList(new Pair<Node, Object>(
					FreeplanePlugin.getInstance().getFreeplaneUtils().getEmptyNode(nodeModel), nodeModel));
		}
		List<Pair<Node, Object>> children = new ArrayList<Pair<Node, Object>>();		
		for (NodeModel child : nodeModel.getChildren()) {
			children.add(new Pair<Node, Object>(
					FreeplanePlugin.getInstance().getFreeplaneUtils().getEmptyNode(child), child));
		}
