...
public class FreeplanePlugin extends AbstractFlowerJavaPlugin {
	...
	public void start(BundleContext bundleContext) throws Exception {
		...
		TypeDescriptor freeplaneNodeDescriptor = CorePlugin.getInstance().getNodeTypeDescriptorRegistry().getOrCreateTypeDescriptor(FREEPLANE_NODE_TYPE);
		freeplaneNodeDescriptor.addAdditiveController(ADD_CHILD_DESCRIPTOR, new AddChildDescriptor().setChildTypeAs(FREEPLANE_NODE_TYPE).setLabelAs(getMessage("freeplane.add")));
		...
		mmTypeDescriptor.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor().setNameAs(TYPE));
		mmTypeDescriptor.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor().setNameAs(TEXT));
