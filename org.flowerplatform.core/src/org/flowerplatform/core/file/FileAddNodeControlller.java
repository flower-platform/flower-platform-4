 ...
 */
public class FileAddNodeControlller extends AddNodeController {
		...
		public void addNode(Node parentNode, Node child) {
			...
			try {
				if (parentNode.getType().equals("fileSystem")) {
					parentFile = fileAccessController.getFile("d:\\temp\\fileSystemNode");
				} else {
			...
			String name = ((HashMap)(child.getProperties())).get("body").toString();
			Object fileToCreate = fileAccessController.getFile(parentFile, name);
			boolean isDir= (Boolean)((HashMap)(child.getProperties())).get("isDirectory");
			if (isDir) {
