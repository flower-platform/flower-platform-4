...
//		repositoriesService.applyExtension(login, repoName, "git");
		...
		List<String> members = (List<String>) repository.getProperties().get(CoreConstants.MEMBERS);
//		List<String> memberss = (List<String>) repository.getPropertyValue(CoreConstants.MEMBERS);
		List<String> extensions = (List<String>) repository.getProperties().get(CoreConstants.EXTENSIONS);
		List<String> extensionss = (List<String>) repository.getPropertyValue(CoreConstants.EXTENSIONS);
		
//		List<ExtensionInfoInFile> extensions = (List<ExtensionInfoInFile>) repository.getPropertyValue(CoreConstants.EXTENSIONS);
		...
		for (String extensionDependency : extensionMetadata.getDependencies()) {
			...
			if (i == extensions.size()) {
				fail(String.format("Dependency '%s' not found in dependencies, although '%s' depends on it!",
						extensions.get(i), extensionMetadata.getLabel()));
			}
