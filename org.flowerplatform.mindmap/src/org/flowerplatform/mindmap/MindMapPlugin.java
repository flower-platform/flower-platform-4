...
public class MindMapPlugin extends AbstractFlowerJavaPlugin {
	...
	public void start(BundleContext bundleContext) throws Exception {
		...
		.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor().setTypeAs("MindMapIconsWithButton").setNameAs(CoreConstants.ICONS).setTitleAs(getMessage("mindmap.icons")).setReadOnlyAs(false))	
		...
		.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor().setTypeAs(PROPERTY_DESCRIPTOR_TYPE_DROP_DOWN_LIST).setNameAs(CLOUD_SHAPE).setTitleAs(getMessage("mindmap.cloud.shape")).setPossibleValuesAs(Arrays.asList("", "Rectangle", "Round Rectangle")).setReadOnlyAs(false).setCategoryAs(getMessage("mindmap.cloud")))
