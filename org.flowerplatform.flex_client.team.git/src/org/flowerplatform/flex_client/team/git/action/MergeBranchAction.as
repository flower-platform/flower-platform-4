	...
	public class MergeBranchAction extends ActionBase{
		...
		override public function get visible():Boolean {
			if (selection != null && selection.length == 1 && selection.getItemAt(0) is Node){
				...
				if(node.type == GitConstants.GIT_LOCAL_BRANCH_TYPE || node.type == GitConstants.GIT_REPO_TYPE) {
					label = Resources.getMessage("flex_client.team.git.action.mergeBranch.label");
					return true;
				}
				if(node.type == GitConstants.GIT_REMOTE_BRANCH_TYPE) {
		...
		public function showPopUp(node:Node):void {
		...
		override public function run():void {
			var node:Node = Node(selection.getItemAt(0));
			/* remove this when checkout property is finished
			if(node.type == GitConstants.GIT_REPO_TYPE) {
				for each(var childGit:Node in node.children) {
					for each(var child:Node in childGit.children) {
						if(child.getPropertyValue(GitConstants.IS_CHECKOUT) == "true"){
							showPopUp(child);
						}
					}
				}
			...
			//if(node.getPropertyValue(GitConstants.IS_CHECKOUT) == "true" && node.type == GitConstants.GIT_LOCAL_BRANCH_TYPE) {
			//remove this when checkout property is finished 
			if(node.type == GitConstants.GIT_LOCAL_BRANCH_TYPE || node.type == GitConstants.GIT_REPO_TYPE) {
				showPopUp(node);
			}
			
			//if(node.getPropertyValue(GitConstants.IS_CHECKOUT) == "false" && (node.type == GitConstants.GIT_REMOTE_BRANCH_TYPE || node.type == GitConstants.GIT_LOCAL_BRANCH_TYPE)) {
			//remove this when checkout property is finished
			if(node.type == GitConstants.GIT_REMOTE_BRANCH_TYPE) {
