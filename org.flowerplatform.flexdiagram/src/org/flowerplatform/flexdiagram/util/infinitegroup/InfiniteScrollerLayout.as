...
package org.flowerplatform.flexdiagram.util.infinitegroup {
	...
	/**
	 * Copy/Paste class from ScrollerLayout (this one is excluded).
	 * 
	 * <p>
	...
	public class InfiniteScrollerLayout extends LayoutBase {
		...
		public function InfiniteScrollerLayout() {
			super();
		}
		 ...
		 *  @private
		 */
		private function getScroller():InfiniteScroller {
			var g:Skin = target as Skin;
			return (g && ("hostComponent" in g)) ? Object(g).hostComponent as InfiniteScroller : null;
		}
		...
		private function set hsbVisible(value:Boolean):void {
			if (value && getScroller().viewport.scrollByToolInProgress) {
				value = !value;
			}
		...
		private function hsbRequiredHeight():Number {
			var scroller:InfiniteScroller = getScroller();
		...
		private function set goBackButtonVisible(value:Boolean):void {
			if (value && getScroller().viewport.scrollByToolInProgress) {
				value = !value;
			}
			var goBackButton:Button = getScroller().goBackButton;
			if (!goBackButton || goBackButton.visible == value)
				return;
			
			goBackButton.includeInLayout = goBackButton.visible = value;
		}
		...
		override public function measure():void {
			const scroller:InfiniteScroller = getScroller();
			if (!scroller) 
				return;
			
			const minViewportInset:Number = 0;
			const measuredSizeIncludesScrollBars:Boolean = (scroller.getStyle("interactionMode") == InteractionMode.MOUSE);
		...
		override public function updateDisplayList(w:Number, h:Number):void {  
			...
			var hsb:ScrollBarBase = scroller.horizontalScrollBar;
			var vsb:ScrollBarBase = scroller.verticalScrollBar;
			var goBackBtn:GoBackButton = scroller.goBackButton;
			var minViewportInset:Number = 0;
			...
			if (goBackBtn.visible) {
				switch (goBackBtn.position) {
					case GoBackButton.TOP_LEFT:						
						goBackBtn.setLayoutBoundsPosition(0, 0);
						break;
					case GoBackButton.TOP_RIGHT:
						goBackBtn.setLayoutBoundsPosition(
							w - goBackBtn.getPreferredBoundsWidth() - (vsbVisible ? vsb.getPreferredBoundsWidth() : 0), 
							0);
						break;
					case GoBackButton.BOTTOM_LEFT:
						goBackBtn.setLayoutBoundsPosition(
							0, 
							h - goBackBtn.getPreferredBoundsHeight() - (hsbVisible ? hsb.getPreferredBoundsHeight() : 0));
						break;
					case GoBackButton.BOTTOM_RIGHT:
						goBackBtn.setLayoutBoundsPosition(
							w - goBackBtn.getPreferredBoundsWidth() - (vsbVisible ? vsb.getPreferredBoundsWidth() : 0),
							h - goBackBtn.getPreferredBoundsHeight() - (hsbVisible ? hsb.getPreferredBoundsHeight() : 0));
						break;
					case GoBackButton.LEFT:
						goBackBtn.setLayoutBoundsPosition(
							0, 
							(h - (hsbVisible ? hsb.getPreferredBoundsHeight() : 0))/2 - goBackBtn.getPreferredBoundsHeight()/2);
						break;
					case GoBackButton.RIGHT:
						goBackBtn.setLayoutBoundsPosition(
							w - goBackBtn.getPreferredBoundsWidth() - (vsbVisible ? vsb.getPreferredBoundsWidth() : 0), 
							(h - (hsbVisible ? hsb.getPreferredBoundsHeight() : 0))/2);
						break;
					case GoBackButton.TOP:
						goBackBtn.setLayoutBoundsPosition(
							(w - (vsbVisible ? vsb.getPreferredBoundsWidth() : 0))/2 - goBackBtn.getPreferredBoundsWidth()/2, 
							0);
						break;
					case GoBackButton.BOTTOM:
						goBackBtn.setLayoutBoundsPosition(
							(w - (vsbVisible ? vsb.getPreferredBoundsWidth() : 0))/2 - goBackBtn.getPreferredBoundsWidth()/2,
							h - goBackBtn.getPreferredBoundsHeight() - (hsbVisible ? hsb.getPreferredBoundsHeight() : 0));
						break;
