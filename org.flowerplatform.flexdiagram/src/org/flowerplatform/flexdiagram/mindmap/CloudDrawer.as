package org.flowerplatform.flexdiagram.mindmap {
	...
	*/
	public class CloudDrawer {
		...
		public function drawCloud(renderer:AbstractMindMapNodeRenderer, unscaledWidth:Number, unscaledHeight:Number):void {			
			if (renderer._cloudType != null && renderer._cloudType != renderer.CLOUD_TYPE_NONE) {
				renderer.graphics.lineStyle(2, 0x808080); // gray line with bigger thickness
				renderer.graphics.beginFill(Utils.convertValueToColor(renderer._cloudColor), 1);
				...
				if (renderer._cloudType == renderer.CLOUD_TYPE_RECTANGLE) {
					renderer.graphics.drawRect(shapeX, shapeY, shapeWidth, shapeHeight);
				} else if (renderer._cloudType == renderer.CLOUD_TYPE_ROUNDED_RECTANGLE){
					renderer.graphics.drawRoundRect(shapeX, shapeY, shapeWidth, shapeHeight, 25, 25);					
				} else if (renderer._cloudType == renderer.CLOUD_TYPE_ARC || renderer._cloudType == renderer.CLOUD_TYPE_STAR) {
					paint(renderer);
				}
		...
		public function paint(renderer:AbstractMindMapNodeRenderer):void {
		...
		public function paintDecoration(renderer:AbstractMindMapNodeRenderer, p:ArrayCollection):void {
			var middleDistanceBetweenPoints:Number;
			if (renderer._cloudType == renderer.CLOUD_TYPE_ARC) {
				middleDistanceBetweenPoints = 120;
			} else if (renderer._cloudType == renderer.CLOUD_TYPE_STAR) {
		...
		public function paintDeco(renderer: AbstractMindMapNodeRenderer, x0:Number, y0:Number, x1:Number, y1:Number):void {
			var dx:Number;
			var dy:Number;
			dx = x1 - x0;
			dy = y1 - y0;
		...
		public function paintD(renderer:AbstractMindMapNodeRenderer, x0:Number, y0:Number, x1:Number, y1:Number, dx:Number, dy:Number, dxn:Number, dyn:Number):void {
			var xctrl:Number;
			var yctrl:Number;
		...
		public function getCoordinates(renderer:AbstractMindMapNodeRenderer, inList:ArrayCollection, node:Object):void {
