...
var NodeRegistryManager = function(resourceOperationsHandler, hostServiceInvocator, hostInvocator) {
	this.resourceOperationsManager = new ResourceOperationsManager(this, resourceOperationsHandler);
	this.hostServiceInvocator = hostServiceInvocator;
	this.hostInvocator = hostInvocator;
...
NodeRegistryManager.prototype.expand = function(nodeRegistry, node, context) {
	if (!node.properties[Constants.AUTO_SUBSCRIBE_ON_EXPAND]) {
...
NodeRegistryManager.prototype.getResourceUrisForSubTree = function(node, nodeRegistry, dirtyResourceUris, savedResourceUris) {
	...
	if (node.children != null) {
		for (var i = 0; i < this.hostInvocator.getLength(node.children); i++){	
			this.getResourceUrisForSubTree(this.hostInvocator.getItemAt(node.children, i), nodeRegistry, dirtyResourceUris, savedResourceUris);
		}
	}
};
...
NodeRegistryManager.prototype.subscribe = function(nodeId, nodeRegistry, subscribeResultCallback, subscribeFaultCallback) {
	...
	this.hostServiceInvocator.invoke("resourceService.subscribeToParentResource", [nodeId], 
		...
		function(fault) {
			self.hostInvocator.showMessageBox("editor.error.subscribe.title", "editor.error.subscribe.message", [fault.faultString]);
...
NodeRegistryManager.prototype.unlinkResourceNodesForcefully = function(resourceUris) {
	...
	this.hostInvocator.showMessageBox("editor.error.subscribe.title", "editor.error.subscribe.message", [idsList]);
};
...
var Constants = {IS_DIRTY: "isDirty", SUBSCRIBABLE_RESOURCES: "subscribableResources", AUTO_SUBSCRIBE_ON_EXPAND: "autoSubscribeOnExpand", UPDATED: "UPDATED", ADDED: "ADDED", REMOVED: "REMOVED", REQUEST_REFRESH: "REQUEST_REFRESH"};
