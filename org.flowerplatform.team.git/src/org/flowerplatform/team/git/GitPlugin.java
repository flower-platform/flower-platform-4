	...
	public void start(BundleContext bundleContext) throws Exception {
		...
		CorePlugin.getInstance().getNodeTypeDescriptorRegistry().getOrCreateCategoryTypeDescriptor(GIT_REF)
			...
			.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor()
				.setNameAs(NAME).setTitleAs(ResourcesPlugin.getInstance().getMessage("git.name")).setOrderIndexAs(0))
			.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor()
				.setNameAs(FULL_NAME).setTitleAs(ResourcesPlugin.getInstance().getMessage("git.fullName")).setReadOnlyAs(true).setOrderIndexAs(1))
			.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor()
				.setNameAs(COMMIT_ID).setTitleAs(ResourcesPlugin.getInstance().getMessage("git.commitID")).setReadOnlyAs(true).setOrderIndexAs(2))
			.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor()
				.setNameAs(COMMIT_MESSAGE).setTitleAs(ResourcesPlugin.getInstance().getMessage("git.commitMessage")).setReadOnlyAs(true).setOrderIndexAs(3))
			.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor()
				.setNameAs(IS_CHECKEDOUT).setTitleAs(ResourcesPlugin.getInstance().getMessage("git.isCheckout"))
				.setTypeAs(PROPERTY_DESCRIPTOR_TYPE_BOOLEAN).setReadOnlyAs(true).setOrderIndexAs(4))
			.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor()
				.setNameAs(CONFIG_UPSTREAM_BRANCH).setTitleAs(ResourcesPlugin.getInstance().getMessage("git.configUpstreamBranch")).setReadOnlyAs(true).setOrderIndexAs(5))
			.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor()
				.setNameAs(CONFIG_REMOTE).setTitleAs(ResourcesPlugin.getInstance().getMessage("git.configRemote")).setReadOnlyAs(true).setOrderIndexAs(6))
			.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor()
				.setNameAs(CONFIG_REBASE).setTitleAs(ResourcesPlugin.getInstance().getMessage("git.configRebase"))
				.setTypeAs(PROPERTY_DESCRIPTOR_TYPE_BOOLEAN).setReadOnlyAs(true).setOrderIndexAs(7));
		...
		CorePlugin.getInstance().getNodeTypeDescriptorRegistry().getOrCreateTypeDescriptor(GIT_REPO_TYPE)
			...
			.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor()
				.setNameAs(NAME).setTitleAs(ResourcesPlugin.getInstance().getMessage("git.name")).setOrderIndexAs(0))
			.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor()
				.setNameAs(CURRENT_BRANCH).setTitleAs(ResourcesPlugin.getInstance().getMessage("git.currentBranch")).setReadOnlyAs(true).setOrderIndexAs(1))
			.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor()
				.setNameAs(CURRENT_COMMIT).setTitleAs(ResourcesPlugin.getInstance().getMessage("git.currentCommit")).setReadOnlyAs(true).setOrderIndexAs(2))
		...
		CorePlugin.getInstance().getNodeTypeDescriptorRegistry().getOrCreateTypeDescriptor(GIT_LOCAL_BRANCHES_TYPE)
			...
			.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor()
				.setNameAs(NAME).setTitleAs(ResourcesPlugin.getInstance().getMessage("git.name")).setOrderIndexAs(0))
			.addAdditiveController(CHILDREN_PROVIDER, new GitRefsChildrenProvider(Constants.R_HEADS, GIT_SCHEME, GIT_LOCAL_BRANCH_TYPE))
			.addCategory(GIT_CATEGORY);
		...
		CorePlugin.getInstance().getNodeTypeDescriptorRegistry().getOrCreateTypeDescriptor(GIT_REMOTE_BRANCHES_TYPE)
			...
			.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor()
				.setNameAs(NAME).setTitleAs(ResourcesPlugin.getInstance().getMessage("git.name")).setOrderIndexAs(0))
			.addAdditiveController(CHILDREN_PROVIDER, new GitRefsChildrenProvider(Constants.R_REMOTES, GIT_SCHEME, GIT_REMOTE_BRANCH_TYPE))
			.addCategory(GIT_CATEGORY);
		...
		CorePlugin.getInstance().getNodeTypeDescriptorRegistry().getOrCreateTypeDescriptor(GIT_TAGS_TYPE)
			...
			.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor()
				.setNameAs(NAME).setTitleAs(ResourcesPlugin.getInstance().getMessage("git.name")).setOrderIndexAs(0))
			.addAdditiveController(CHILDREN_PROVIDER, new GitRefsChildrenProvider(Constants.R_TAGS, GIT_SCHEME, GIT_TAG_TYPE))
			.addCategory(GIT_CATEGORY);
		...
		CorePlugin.getInstance().getNodeTypeDescriptorRegistry().getOrCreateTypeDescriptor(GIT_REMOTE_TYPE)
			...
			.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor()
				.setNameAs(NAME).setTitleAs(ResourcesPlugin.getInstance().getMessage("git.name")).setOrderIndexAs(0))
			.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor()
				.setNameAs(FETCH_REF_SPECS).setTitleAs(ResourcesPlugin.getInstance().getMessage("git.fetchRefSpecs")).setReadOnlyAs(true).setOrderIndexAs(1))
			.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor()
				.setNameAs(PUSH_REF_SPECS).setTitleAs(ResourcesPlugin.getInstance().getMessage("git.pushRefSpecs")).setReadOnlyAs(true).setOrderIndexAs(2))
			.addAdditiveController(PROPERTY_DESCRIPTOR, new PropertyDescriptor()
				.setNameAs(REMOTE_URIS).setTitleAs(ResourcesPlugin.getInstance().getMessage("git.remoteURIs")).setReadOnlyAs(true).setOrderIndexAs(3));
